        ...
        <div className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl border shadow-sm ${isDark ? 'bg-zinc-900/90 border-zinc-800' : 'bg-white border-zinc-200'}`}>
          ...
          {submitted ? (
            <div className="py-8 text-center space-y-4">
              ...
              <div>
                <h3 className="text-base font-bold text-emerald-500">{t('pub.inquiryPage.successTitle', 'Demande envoyée avec succès !')}</h3>
                <p className="text-xs text-zinc-400 max-w-sm mx-auto mt-1">
                  {t('pub.inquiryPage.successDesc', 'Notre équipe va examiner votre demande et configurer votre campagne dans le Dashboard PUB. Vous recevrez une notification dès validation.')}
                </p>
              </div>
              <div className="pt-3 flex gap-3 justify-center">
                ...
                >
                  {t('pub.inquiryPage.sendAnother', 'Envoyer une autre demande')}
                </button>
                <button
                  onClick={() => navigate('/pro')}
                  className="px-5 py-2 rounded-xl text-xs font-bold bg-[#FF6B00] text-white hover:bg-[#e05e00]"
                >
                  {t('pub.inquiryPage.backHome', "Retour à l'accueil")}
                </button>
              </div>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4 pt-2">
              <div>
                <label className="text-xs font-bold text-zinc-400 block mb-1">{t('pub.modal.companyName', 'Nom de votre entreprise / Marque *')}</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-zinc-400 block mb-1">{t('pub.modal.contactName', 'Nom du contact *')}</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="text-xs font-bold text-zinc-400 block mb-1">{t('pub.modal.sector', "Secteur d'activité *")}</label>
                  <select
                    value={form.sector}
                  ...
                  >
                    <option value="Technologie">{t('pub.sectors.tech', 'Technologie & IT')}</option>
                    <option value="Finance">{t('pub.sectors.finance', 'Finance & Fintech')}</option>
                    <option value="Commerce">{t('pub.sectors.commerce', 'Commerce & Distribution')}</option>
                    <option value="Santé">{t('pub.sectors.health', 'Santé & Médical')}</option>
                    <option value="Éducation">{t('pub.sectors.education', 'Éducation & Formation')}</option>
                    <option value="Services">{t('pub.sectors.services', 'Services Professionnels')}</option>
                    <option value="Autre">{t('pub.sectors.other', 'Autre secteur')}</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-zinc-400 block mb-1">{t('pub.modal.phone', 'Numéro Téléphone / WhatsApp *')}</label>
                  <input
                    type="tel"
                    required
                ...
                <div>
                  <label className="text-xs font-bold text-zinc-400 block mb-1">{t('pub.modal.email', 'Email professionnel')}</label>
                  <input
                    type="email"
                    value={form.email}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-zinc-400 block mb-1">{t('pub.inquiryPage.budgetEnvisaged', 'Budget envisagé')}</label>
                  <input
                    type="number"
                    min="100"
                    value={form.budget}
                ...
                <div>
                  <label className="text-xs font-bold text-zinc-400 block mb-1">{t('pub.inquiryPage.currency', 'Devise')}</label>
                  <select
                    value={form.currency}
                  ...
                  >
                    <option value="HTG">{t('pub.currencies.htg', 'Gourde (HTG)')}</option>
                    <option value="USD">{t('pub.currencies.usd', 'Dollar (USD)')}</option>
                    <option value="EUR">{t('pub.currencies.eur', 'Euro (EUR)')}</option>
                    <option value="CAD">{t('pub.currencies.cad', 'Dollar Canadien (CAD)')}</option>
                  </select>
                </div>
              </div>

              <div>
                <label className="text-xs font-bold text-zinc-400 block mb-1">{t('pub.inquiryPage.messageObjective', 'Message ou objectif de la publicité')}</label>
                <textarea
                  rows={3}
                  value={form.message}
                  onChange={e => setForm({ ...form, message: e.target.value })}
                  placeholder={t('pub.inquiryPage.messagePlaceholder', 'Décrivez brièvement le produit, service ou événement que vous souhaitez promouvoir...')}
                  className={`w-full px-3.5 py-2.5 rounded-xl border text-sm ${
                    isDark ? 'bg-zinc-800/80 border-zinc-700 text-white' : 'bg-zinc-50 border-zinc-200 text-zinc-900'
                  } focus:outline-none focus:ring-2 focus:ring-[#FF6B00]`}
                />
              ...
              >
                <Send size={16} />
                <span>{isSubmitting ? t('pub.inquiryPage.submitting', 'Envoi en cours...') : t('pub.inquiryPage.submitBtn', 'Envoyer la demande publicitaire')}</span>
              </button>
            </form>
          )}
        </div>
