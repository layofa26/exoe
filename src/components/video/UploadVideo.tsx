...
type UploadStep = 'select' | 'details' | 'options' | 'review' | 'compressing' | 'uploading' | 'processing' | 'success' | 'error'
...
export const UploadVideo = ({ isOpen = false, onClose, initialVideoData, onSuccess }: UploadVideoProps): JSX.Element => {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (!finalCover && fileToUpload) {
        const auto = await captureCoverFromVideo(fileToUpload)
        if (auto) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-[11px] ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
                  ...
                  {currentStep === 'compressing' && t('pro.upload.compressing', 'Compression et optimisation de la vidéo...')}
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">
            ...
            {currentStep === 'compressing' && (
              <div className="py-12 text-center space-y-4">
                ...
                <div>
                  <h3 className="text-base font-bold mb-1">
                    {t('pro.upload.compressingTitle', 'Compression et optimisation de la vidéo...')}
                  </h3>
                  <p className={`text-xs ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
                    {compressionProgress}% {t('pro.upload.compressionProcessed', 'optimisé pour réduire la taille')}
                  </p>
                </div>
                ...
                <p className={`text-[11px] ${isDark ? 'text-zinc-500' : 'text-slate-400'} pt-2`}>
                  {t('pro.upload.compressingTip', 'Cette étape réduit la taille de votre vidéo avant l\'envoi pour économiser vos données et le stockage.')}
                </p>
              </div>
            )}
              ...
              <div className="py-12 text-center space-y-4">
                ...
                <div>
                  ...
                  {compressionSavings !== null && compressionSavings > 0 && (
                    <span className="inline-block mt-1 px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                      ⚡ Vidéo allégée de {compressionSavings}%
                    </span>
                  )}
                </div>
